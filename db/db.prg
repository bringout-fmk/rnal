...
if (nArea==-1 .or. nArea == nArea2)
	...
	do case 
		case (nArea2 == F_DOCS) .or. (nArea2 == F__DOCS)
			...
			CREATE_INDEX("2", "STR(doc_priority,4)+DESCEND(DTOS(doc_date))+STR(doc_no,10)", cPath + cTable, .t.)
			CREATE_INDEX("3", "STR(doc_priority,4)+DESCEND(DTOS(doc_dvr_date))+STR(doc_no,10)", cPath + cTable, .t.)
